...
function enqueue_custom_styles_scripts() {
	...
	wp_localize_script('custom-script', 'ajax_object', array(
		...
		'nonce' => wp_create_nonce('ajax-items-table-call')
	));
}
...
function search_and_filter_items($initial_load = false) {
    $search_term = isset($_REQUEST['search_term']) ? $_REQUEST['search_term'] : '';
    $item_category = isset($_REQUEST['item_category']) ? $_REQUEST['item_category'] : 'all';
    $first_letter = isset($_REQUEST['first_letter']) ? $_REQUEST['first_letter'] : 'all';
...
if ($initial_load) {
    ...
    check_ajax_referer('ajax-items-table-call', '_wpnonce', true);
}
...
function items_table_shortcode($atts) {
        ...
        <table id="items_table">
            ...
            <tbody>
                <?php echo search_and_filter_items(true); ?>
            </tbody>
        </table>
